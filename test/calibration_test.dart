...
void main() {
  ...
  group('discipline', () {
    ...
    test(
      'ignores in-progress topics — cannot compare partial work honestly',
      () {
  ...
  group('genuine signal', () {
    ...
    test(
      'applies to remaining topics only, not the ones the evidence came from',
      () {
        ...
        final s = cal
            .analyse(topics: [...past.topics, ...open], completed: past.logs)
            .single;
        expect(s.affectedTopicIds.toSet(), {'open0', 'open1', 'open2'});
      },
    );
  });
  ...
  group('grouping', () {
    test('recommends per (subject, unit) — never mixes them', () {
      ...
      final problems = finished(
        prefix: 'q',
        subjectId: 'phys',
        unit: EffortUnit.problems,
        amount: 20,
        rate: 6.0,
        actualMinutesEach: 60,
        n: 6,
      );
      ...
      expect(
        pagesRec.affectedTopicIds.every((id) => id.startsWith('openP')),
        isTrue,
      );
      expect(
        probsRec.affectedTopicIds.every((id) => id.startsWith('openQ')),
        isTrue,
      );
    });
  });
